...
class HGTLinkPrediction(torch.nn.Module):
    def __init__(self, hidden_channels, out_channels, data=None, metadata=None, dropout=0.5, num_heads=4, num_layers=3):
        ...
        for node_type in node_types:
            # Fallback to standard text feature size of 768 if data is not present to read size from
...
def train_one_config(split_data, config, device, final_mode=False, trial=None):
    ...
    model = HGTLinkPrediction(hidden_dim, OUTPUT_DIM, data=train_data, dropout=dropout).to(device)
